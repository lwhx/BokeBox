...
- **Motion mode**: derive chapters and animation count from the spoken script, generate SRT during audio synthesis, and create a different HTML composition and entrance motion for every shot; let creators constrain the base, palette, type, information density, and motion intensity; preview in the player or open a standalone page for recording (see [Motion mode](motion-mode.md))
