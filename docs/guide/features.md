...
- **Motion 模式**：根据口播稿长度动态规划章节与动画数量，音频阶段生成 SRT，为每个镜头生成不同 HTML 构图和入场动效；支持用户结构化控制基底、配色、字形、信息密度与动效强度；播放器内可预览，也可新页全屏录制（见 [Motion 模式](motion-mode.md)）
- 专辑：创建 / 整理 / 专辑内连续收听
- 图谱标签浏览：用搜索快速定位主题，查看高频主题与共现连线，再按主题回到相关节目
- 听播库与制作任务同空间管理
