...
- 星图页重设计为「天图星表」风格：光谱色温星点 + 亮星衍射十字、倾斜银河雾带、天球经纬网格、星座连线（选中星时聚焦共现关联并淡化其余）、细字星表标签 + 引线、细选中环；移除轨道环 / 扫描线 / 极光等科技元素
- 星图 HUD 去毛玻璃面板：标题块与操作芯片改为安静星表样式，亮暗双主题保持一致
- 星图标签由胶囊按钮改为经典图表标注：色点 + 字距小写标题 + 计数，选中聚焦时以星色强调

### 文档

- 设计 Token 同步：星图 HUD 芯片现以 `--tc-ink-*` 混色细边框呈现，不再依赖 `--tc-chip-bg` 实玻璃
