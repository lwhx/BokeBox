...
function findStepAnchorIndices(
  ...
  if (picks.length < needed) {
    ...
    let acc = 0;
    let t = 0;
    for (let i = fromIndex + 1; i < toIndex && t < targets.length; i += 1) {
...
export function buildStoryboard(input: StoryboardInput): StoryboardResult {
  ...
  if (segments) {
    ...
    for (let i = 0; i < cuts.length - 1; i += 1) {
      boundaries.push({
        start: cuts[i],
        end: i + 1 < cuts.length - 1 ? cuts[i + 1] - 1 : lastIdx,
      });
    }
  }
  ...
  for (const b of boundaries) {
    ...
    if (!isLast && merged.length && endMs - (merged[merged.length - 1] as { end: number }).end < MIN_BEAT_MS) {
  ...
  merged.forEach((b, i) => {
    ...
    // 4) steps：2-3 步，锚点钉在语义触发 cue 的 startMs
    const stepCount = Math.min(3, Math.max(2, Math.round((endMs - startMs) / 8_000) + 1));
...
function shortTitleFromCues(cues: SrtCue[], from: number, to: number, max = 22): string {
  const text = cues
    .slice(from, to + 1)
    .map((c) => c.text)
    .join(' ')
