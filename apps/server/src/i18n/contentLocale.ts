...
export function buildPodcastSystemPrompt(input: {
  ...
  if (isZhFamily(loc)) {
    ...
    return [
      ...
      '   motionChapters({id,title,summary,script}[]), script, showNotes, estimatedMinutes(number)。',
      '3. title 像播客单集标题；summary 80-140 字；tags 3-6 个。',
      `4. script 必须是自然口播${langName}，含开场、中段、收尾。`,
      '   同时把 script 按口播内容切成多个 motionChapters：以每章约 220 字作为参考，短稿至少 2 章，长稿按实际字数增加，不要固定为 2–3 章（数量受系统安全上限约束）；保留开场、核心观点、转折和收束。',
      '   每个 motionChapter 的 script 必须是可直接送入 TTS 的真实口播片段；script 必须按章节顺序用空行拼接，不能在口播里加入章节标题或 markdown 标记。',
      '   Motion 会为每个章节生成主视觉页；章节要围绕完整观点或一个自然口播段落切分，不要机械地每句话单独成章。',
      ...
      `8. 所有面向用户的字段（title/summary/tags/hostIntro/outline/motionChapters/script/showNotes）必须使用${langName}。`,
      personaSection || '',
    ]
      .filter(Boolean)
      .join('\n');
  ...
  return [
    ...
    '   Also split script into as many motionChapters as the spoken content needs: use roughly 220 spoken characters per chapter as a guide, keep at least 2 for short scripts, and increase the count for longer scripts; do not force a fixed 2–3 count (the system safety ceiling still applies). Preserve the hook, core points, transitions, and close.',
    '   Each motionChapter.script must be a real spoken fragment ready for TTS. script MUST be the ordered chapter scripts joined with blank lines; do not put chapter titles or markdown markers into spoken text.',
    '   Motion creates one primary visual page per chapter; split around complete ideas or natural spoken paragraphs, not mechanically one chapter per sentence.',
    ...
    `8. ALL user-facing fields (title/summary/tags/hostIntro/outline/motionChapters/script/showNotes) MUST be in ${lang}.`,
    personaSection || '',
  ]
    .filter(Boolean)
    .join('\n');
