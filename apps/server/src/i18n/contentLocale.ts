...
export function buildPodcastSystemPrompt(input: {
  ...
  if (isZhFamily(loc)) {
    ...
    return [
      ...
      '   同时把 script 切成 2-3 个 motionChapters：第一章负责钩子，第二章承载核心观点，第三章负责收束与行动建议（如果内容较短可只返回 2 章）。',
      '   每个 motionChapter 的 script 必须是可直接送入 TTS 的真实口播片段；script 必须按章节顺序用空行拼接，不能在口播里加入章节标题或 markdown 标记。',
      '   Motion 页面只会为这些章节生成 2-3 张主视觉页，不要把每句话拆成章节。',
  ...
  return [
    ...
    '   Also split script into 2-3 motionChapters: a hook, a core section, and a takeaway/close (only 2 when the content is short).',
    ...
    '   Motion will create only 2-3 primary visual pages from these chapters; do not create one chapter per sentence.',
